...
import { 
    ...
    HardDrive, ArrowLeftRight, ArchiveRestore, Kanban, BookOpen, ListTodo, CheckSquare, Repeat, Circle, Megaphone, Info
} from 'lucide-react';
...
function App() {
    ...
    const isInitialLoadAvisos = useRef(true);
    ...
    const [eqForm, setEqForm] = useState({ type: 'Notebook', model: '', specs: '', acquisitionDate: '', warrantyDate: '', price: '', deviceId: '', serialNumber: '', ipAddress: '', status: 'guardado', unit: '', department: '', operator: '', anydeskId: '' });
    ...
    // Acessórios - Views e Filtros
    const [accViewMode, setAccViewMode] = useState('stock'); 
    const [accMoveMonth, setAccMoveMonth] = useState(new Date().getMonth());
    const [accMoveYear, setAccMoveYear] = useState(new Date().getFullYear());
    const [isAccMoveMonthMenuOpen, setIsAccMoveMonthMenuOpen] = useState(false);
    const [isAccMoveYearMenuOpen, setIsAccMoveYearMenuOpen] = useState(false);
    const accMoveMonthMenuRef = useRef(null);
    const accMoveYearMenuRef = useRef(null);
    const [accMoveTypeFilter, setAccMoveTypeFilter] = useState('Todos');
    const [isAccMoveTypeMenuOpen, setIsAccMoveTypeMenuOpen] = useState(false);
    const accMoveTypeMenuRef = useRef(null);
    const [expandedAccGroups, setExpandedAccGroups] = useState({});
            ...
            if (isAdmin) {
                if (view === 'reports') {
                    ...
                    try {
                        ...
                        else if (t.createdAt) ticketDate = new Date(t.createdAt);
    ...
    useEffect(() => {
        ...
        if (view === 'reports' && isAdmin && window.Chart && lineChartRef.current && pieChartRef.current) {
            ...
            if (reportFilterMode === 'today') {
                ...
                filteredTickets.forEach(t => {
                    ...
                    if(d && !isNaN(d.getTime())) hoursData[d.getHours()] += 1;
                });
                lineLabels.push(...Array.from({length: 24}, (_, i) => `${i}h`));
                lineData.push(...hoursData);
            } else if (isYearly) {
                ...
                filteredTickets.forEach(t => {
                    ...
                    if(d && !isNaN(d.getTime())) monthsData[d.getMonth()] += 1;
                });
                lineLabels.push(...MONTHS.map(m => m.substring(0,3)));
                lineData.push(...monthsData);
            } else {
                const currentMonthForCalc = selectedMonth;
                const currentYearForCalc = selectedYear;
                const daysInMonth = new Date(currentYearForCalc, currentMonthForCalc + 1, 0).getDate();
                ...
                lineLabels.push(...Array.from({length: daysInMonth}, (_, i) => `${i+1}`));
                lineData.push(...daysData);
            }

            const problemas = filteredTickets.filter(t => t.type !== 'request').length;
            const solicitacoes = filteredTickets.filter(t => t.type === 'request').length;
            ...
            pieChart = new window.Chart(ctxPie, {
                type: 'doughnut',
                data: {
                    labels: ['Problemas Técnicos', 'Solicitações'],
                    datasets: [{
                        data: [problemas, solicitacoes],
                ...
                options: {
                    ...
                    cutout: '65%',
                    plugins: {
                        legend: { position: 'bottom', labels: { color: textColor, usePointStyle: true, padding: 15, font: { size: 10 } } }
                    }
                }
            });
        }
    ...
    }, [view, filteredTickets, isDarkMode, reportFilterMode, selectedMonth, selectedYear, isAdmin]);
        ...
        const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
            ...
            if (currentUser) {
                ...
                if (isTech) {
                    ...
                    setView(v => (v === 'dashboard' || v === 'inventory' || v === 'reports' || v === 'archive' || v === 'kanban' || v === 'checklists') ? 'landing' : v);
                }
            } else {
                setIsAdmin(false);
                localStorage.removeItem('helpdesk_is_admin');
    ...
    const requestNotificationPermission = async () => {
        if ("Notification" in window && Notification.permission !== "granted" && Notification.permission !== "denied") {
            try { await Notification.requestPermission(); } catch (e) { console.warn("Erro ao pedir permissão de notificação:", e); }
        }
    };
    ...
    const showSystemNotification = (title, body) => {
        if ("Notification" in window && Notification.permission === "granted") {
            try { 
                ...
                if (navigator.serviceWorker) { navigator.serviceWorker.ready.then(reg => reg.showNotification(title, { body })); }
            }
        }
    };
    ...
    useEffect(() => {
        ...
        const unsubAvisos = onSnapshot(collection(db, 'avisos'), (snapshot) => {
            setAnnouncements(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })).sort((a,b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)));
            
            if (!isInitialLoadAvisos.current) {
                snapshot.docChanges().forEach((change) => {
                    if (change.type === "added") {
                        const data = change.doc.data();
                        if (data.createdBy !== techName) {
                            playSound('update');
                            showToast(`Novo Aviso Global: ${data.title}`, data.type === 'critical' ? 'error' : 'warning');
                            showSystemNotification(`⚠️ Aviso TI: ${data.title}`, data.message);
                        }
                    }
                });
            } else {
                isInitialLoadAvisos.current = false;
            }
        }, (error) => { console.warn("Erro avisos:", error); setAnnouncements([]); });
    ...
    useEffect(() => {
        function handleClickOutside(event) {
            ...
            if (accMoveMonthMenuRef.current && !accMoveMonthMenuRef.current.contains(event.target)) setIsAccMoveMonthMenuOpen(false);
            if (accMoveYearMenuRef.current && !accMoveYearMenuRef.current.contains(event.target)) setIsAccMoveYearMenuOpen(false);
            if (accMoveTypeMenuRef.current && !accMoveTypeMenuRef.current.contains(event.target)) setIsAccMoveTypeMenuOpen(false);
        }
    ...
    useEffect(() => {
        ...
        const unsubKanban = onSnapshot(collection(db, 'kanban_cards'), (snapshot) => {
            ...
            if (error.code === 'permission-denied') {
                ...
                setFirebaseError({ title: "Acesso ao Kanban/KB/Checklists/Avisos Bloqueado", message: "Adicione as regras das novas coleções no seu Console Firebase (Firestore > Regras).", code: `match /kanban_cards/{doc=**} {\n  allow read, write: if request.auth != null;\n}\nmatch /kb_cards/{doc=**} {\n  allow read, write: if request.auth != null;\n}\nmatch /checklists/{doc=**} {\n  allow read, write: if request.auth != null;\n}\nmatch /avisos/{doc=**} {\n  allow read, write: if request.auth != null;\n}` });
            }
        });
    ...
    const paginatedTickets = useMemo(() => {
        return view === 'archive' ? filteredTickets.slice(0, archiveDisplayLimit) : filteredTickets;
    }, [filteredTickets, view, archiveDisplayLimit]);

    useEffect(() => {
        setArchiveDisplayLimit(10);
    }, [view, searchTerm, unitFilter, priorityFilter, typeFilter]);
    ...
    const filteredEquipments = useMemo(() => equipments.filter(eq => {
        ...
        return (eqTypeFilter === 'Todos' || eq.type === eqTypeFilter) && 
               ...
               (!eqSearchTerm || (eq.model && eq.model.toLowerCase().includes(eqSearchTerm.toLowerCase())) || (eq.deviceId && eq.deviceId.toLowerCase().includes(eqSearchTerm.toLowerCase())) || (eq.serialNumber && eq.serialNumber.toLowerCase().includes(eqSearchTerm.toLowerCase())) || (eq.ipAddress && eq.ipAddress.toLowerCase().includes(eqSearchTerm.toLowerCase())));
    }), [equipments, eqTypeFilter, eqUnitFilter, eqStatusFilter, eqSearchTerm]);

    const decommissionedEquipments = useMemo(() => equipments.filter(eq => eq.status === 'baixado' && (!eqSearchTerm || (eq.model && eq.model.toLowerCase().includes(eqSearchTerm.toLowerCase())) || (eq.deviceId && eq.deviceId.toLowerCase().includes(eqSearchTerm.toLowerCase())) || (eq.serialNumber && eq.serialNumber.toLowerCase().includes(eqSearchTerm.toLowerCase())) || (eq.ipAddress && eq.ipAddress.toLowerCase().includes(eqSearchTerm.toLowerCase())))), [equipments, eqSearchTerm]);
    ...
    const filteredMovements = useMemo(() => {
        ...
        moves = moves.filter(m => {
            if (isNaN(m.dateObj.getTime())) return false;
            
            const matchesMonth = accMoveMonth === 'all' 
                ? m.dateObj.getFullYear() === accMoveYear 
                : (m.dateObj.getMonth() === accMoveMonth && m.dateObj.getFullYear() === accMoveYear);
                
            const matchesType = accMoveTypeFilter === 'Todos' || m.accType === accMoveTypeFilter;

            return matchesMonth && matchesType;
        });
        return moves.sort((a, b) => b.dateObj - a.dateObj);
    }, [accessories, accMoveMonth, accMoveYear, accMoveTypeFilter]);
    ...
    const groupedMovements = useMemo(() => {
        ...
        filteredMovements.forEach(m => {
            ...
            groups[m.accId].logs.push(m);
            
            const action = m.action;
            if (action.includes('Estoque inicial:')) {
                const match = action.match(/Estoque inicial:\s*(\d+)/);
                if (match) groups[m.accId].entradas += parseInt(match[1], 10);
            } else if (action.includes('Entrada de +')) {
                const match = action.match(/Entrada de \+(\d+)/);
                if (match) groups[m.accId].entradas += parseInt(match[1], 10);
            } else if (action.includes('Saída de -')) {
                const match = action.match(/Saída de -(\d+)/);
                if (match) groups[m.accId].saidas += parseInt(match[1], 10);
            }
        });
        return Object.values(groups).sort((a, b) => a.accName.localeCompare(b.accName));
    }, [filteredMovements]);
    ...
    const handleCreateAnnouncement = async (e) => {
        e.preventDefault(); if (!announcementForm.title.trim() || !announcementForm.message.trim()) return; setIsSubmitting(true);
        try {
            await addDoc(collection(db, 'avisos'), { ...announcementForm, createdBy: techName, createdAt: serverTimestamp() });
            showToast("Aviso publicado!", "success"); setAnnouncementForm({ title: '', message: '', type: 'info' }); setIsAnnouncementModalOpen(false);
        } catch(err) { showToast("Erro ao publicar aviso.", "error"); } setIsSubmitting(false);
    };

    const handleDeleteAnnouncement = (id) => {
        setConfirmationModal({
            message: "Deseja mesmo remover este aviso? Ele sumirá para todos imediatamente.",
            onConfirm: async () => {
                try { await deleteDoc(doc(db, 'avisos', id)); showToast("Aviso removido.", "success"); } 
    ...
    const handleEditEquipment = (eq) => { setEqForm({ type: eq.type || 'Notebook', model: eq.model || '', specs: eq.specs || '', acquisitionDate: eq.acquisitionDate || '', warrantyDate: eq.warrantyDate || '', price: eq.price || '', deviceId: eq.deviceId || '', serialNumber: eq.serialNumber || '', ipAddress: eq.ipAddress || '', status: eq.status || 'guardado', unit: eq.unit || '', department: eq.department || '', operator: eq.operator || '', anydeskId: eq.anydeskId || '' }); setEditingEqId(eq.id); setIsEqModalOpen(true); };
    ...
    const handleSaveEquipment = async (e) => {
        ...
        try {
            ...
            setIsEqModalOpen(false); setEditingEqId(null); setEqForm({ type: 'Notebook', model: '', specs: '', acquisitionDate: '', warrantyDate: '', price: '', deviceId: '', serialNumber: '', ipAddress: '', status: 'guardado', unit: '', department: '', operator: '', anydeskId: '' });
    ...
    return (
        <div className={`h-screen overflow-y-auto overflow-x-hidden flex flex-col font-sans transition-colors duration-300 ${isDarkMode ? 'dark bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`} style={{ colorScheme: isDarkMode ? 'dark' : 'light', '--scrollbar-track': isDarkMode ? '#020617' : '#f8fafc', '--scrollbar-thumb': isDarkMode ? '#334155' : '#cbd5e1', '--scrollbar-hover': isDarkMode ? '#475569' : '#94a3b8' }}>
            ...
            {isAnnouncementModalOpen && isAdmin && (<div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200"><div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl border dark:border-slate-800 flex flex-col max-h-[90vh]"><div className="p-6 border-b dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 rounded-t-3xl"><h3 className="text-lg font-bold dark:text-white flex items-center gap-2"><Megaphone size={20} className="text-blue-600"/> Gestão de Avisos Globais</h3><button onClick={() => setIsAnnouncementModalOpen(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors p-2"><X size={24}/></button></div><div className="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-6"><form onSubmit={handleCreateAnnouncement} className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-5 rounded-2xl border dark:border-slate-800"><h4 className="text-sm font-bold dark:text-white mb-2">Publicar Novo Aviso</h4><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setAnnouncementForm({...announcementForm, type: 'info'})} className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 ${announcementForm.type === 'info' ? 'bg-blue-600 text-white shadow-md' : 'bg-white dark:bg-slate-800 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700'}`}><Info size={14}/> Informativo</button><button type="button" onClick={() => setAnnouncementForm({...announcementForm, type: 'warning'})} className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 ${announcementForm.type === 'warning' ? 'bg-orange-500 text-white shadow-md' : 'bg-white dark:bg-slate-800 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700'}`}><AlertTriangle size={14}/> Atenção</button><button type="button" onClick={() => setAnnouncementForm({...announcementForm, type: 'critical'})} className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 ${announcementForm.type === 'critical' ? 'bg-red-600 text-white shadow-md' : 'bg-white dark:bg-slate-800 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700'}`}><AlertOctagon size={14}/> Crítico</button></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Título do Aviso</label><input required type="text" className="w-full px-4 py-3 bg-white dark:bg-slate-900 border dark:border-slate-700 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" placeholder="Ex: Instabilidade no Sistema X" value={announcementForm.title} onChange={(e) => setAnnouncementForm({...announcementForm, title: e.target.value})} /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Mensagem Curta</label><textarea required className="w-full px-4 py-3 bg-white dark:bg-slate-900 border dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white resize-none min-h-[80px]" placeholder="Escreva a mensagem do aviso..." value={announcementForm.message} onChange={(e) => setAnnouncementForm({...announcementForm, message: e.target.value})} /></div><div className="flex justify-end pt-2"><button type="submit" disabled={isSubmitting} className="px-6 py-3 rounded-xl font-black text-sm bg-blue-600 text-white shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all disabled:opacity-50 flex items-center gap-2">{isSubmitting ? <div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"></div> : <Plus size={16}/>} Publicar Aviso</button></div></form><div className="space-y-3"><h4 className="text-sm font-bold dark:text-white flex items-center gap-2"><Layers size={16} className="text-blue-600"/> Avisos Ativos ({announcements.length})</h4>{announcements.length === 0 ? (<p className="text-xs text-slate-500 italic text-center py-6">Nenhum aviso global ativo no momento.</p>) : (announcements.map(aviso => (<div key={aviso.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl border dark:border-slate-700 bg-white dark:bg-slate-800"><div><div className="flex items-center gap-2 mb-1"><span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded text-white ${aviso.type === 'critical' ? 'bg-red-600' : aviso.type === 'warning' ? 'bg-orange-500' : 'bg-blue-600'}`}>{aviso.type === 'critical' ? 'Crítico' : aviso.type === 'warning' ? 'Atenção' : 'Info'}</span><span className="font-bold text-sm dark:text-white">{aviso.title}</span></div><p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2">{aviso.message}</p><p className="text-[9px] text-slate-400 mt-2">Por {aviso.createdBy} • {formatTimestamp(aviso.createdAt)}</p></div><button onClick={() => handleDeleteAnnouncement(aviso.id)} className="flex-shrink-0 flex items-center justify-center p-2.5 bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40 rounded-xl transition-colors" title="Remover Aviso"><Trash2 size={16}/></button></div>)))}</div></div></div></div>)}

            {isEqModalOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200 overflow-y-auto"><div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl border dark:border-slate-800 my-8"><div className="p-6 border-b dark:border-slate-800 flex justify-between items-center"><h3 className="text-xl font-bold dark:text-white flex items-center gap-2"><Monitor size={24} className="text-blue-600"/> {editingEqId ? 'Editar Equipamento' : 'Cadastro de Equipamento'}</h3><button onClick={() => setIsEqModalOpen(false)} className="text-slate-400 hover:text-red-500 transition-colors"><X size={24}/></button></div><div className="p-6"><form onSubmit={handleSaveEquipment} className="space-y-4"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Tipo de Equipamento</label><div className="relative" ref={eqTypeMenuRef}><button type="button" onClick={() => setIsEqTypeMenuOpen(!isEqTypeMenuOpen)} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm font-medium flex items-center justify-between outline-none focus:ring-2 focus:ring-blue-500 transition-all dark:text-white"><span className="truncate">{eqForm.type}</span><ChevronDown size={18} className={`flex-shrink-0 transition-transform ${isEqTypeMenuOpen ? 'rotate-180' : ''}`} /></button>{isEqTypeMenuOpen && (<div className="absolute z-50 mt-2 w-full bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden py-1 animate-in fade-in zoom-in-95 duration-200 custom-scrollbar max-h-48 overflow-y-auto">{EQUIPMENT_TYPES.map(t => (<button key={t} type="button" onClick={() => { setEqForm({...eqForm, type: t}); setIsEqTypeMenuOpen(false); }} className="w-full px-4 py-2 text-left text-sm font-bold hover:bg-blue-50 dark:hover:bg-blue-900/20 dark:text-white flex items-center justify-between">{t} {eqForm.type === t && <Check size={14} className="text-blue-600" />}</button>))}</div>)}</div></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">ID do Dispositivo (IMEI/MAC)</label><input type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium" placeholder="Ex: 00:1A:2B:3C:4D / 1234567890" value={eqForm.deviceId} onChange={(e) => setEqForm({...eqForm, deviceId: e.target.value})} /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Número de Série</label><input type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium" placeholder="Ex: S/N 12345ABC" value={eqForm.serialNumber} onChange={(e) => setEqForm({...eqForm, serialNumber: e.target.value})} /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Modelo / Marca</label><input required type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium" placeholder="Ex: Dell Inspiron 15" value={eqForm.model} onChange={(e) => setEqForm({...eqForm, model: e.target.value})} /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Especificações Técnicas</label><input type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium" placeholder="Ex: i5, 16GB RAM, 512GB SSD" value={eqForm.specs} onChange={(e) => setEqForm({...eqForm, specs: e.target.value})} /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Data de Aquisição</label><input type="text" maxLength={10} placeholder="DD/MM/AAAA" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium" value={eqForm.acquisitionDate} onChange={(e) => { let val = e.target.value.replace(/\D/g, ''); if (val.length > 8) val = val.substring(0, 8); if (val.length > 4) { val = val.replace(/(\d{2})(\d{2})(\d{1,4})/, '$1/$2/$3'); } else if (val.length > 2) { val = val.replace(/(\d{2})(\d{1,2})/, '$1/$2'); } setEqForm({...eqForm, acquisitionDate: val}); }} /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Data de Garantia</label><input type="text" maxLength={10} placeholder="DD/MM/AAAA" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium" value={eqForm.warrantyDate} onChange={(e) => { let val = e.target.value.replace(/\D/g, ''); if (val.length > 8) val = val.substring(0, 8); if (val.length > 4) { val = val.replace(/(\d{2})(\d{2})(\d{1,4})/, '$1/$2/$3'); } else if (val.length > 2) { val = val.replace(/(\d{2})(\d{1,2})/, '$1/$2'); } setEqForm({...eqForm, warrantyDate: val}); }} /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Preço (R$)</label><input type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium" placeholder="R$ 0,00" value={eqForm.price ? parseFloat(eqForm.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : ''} onChange={(e) => { let val = e.target.value.replace(/\D/g, ''); if (!val) { setEqForm({...eqForm, price: ''}); } else { setEqForm({...eqForm, price: (parseInt(val, 10) / 100).toString()}); } }} /></div></div><div className="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/30 space-y-4 mt-4"><p className="text-[10px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 mb-2">Atribuição e Localização (Opcional)</p><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Unidade</label><input type="text" className="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium shadow-sm" placeholder="Ex: Bonway" value={eqForm.unit} onChange={(e) => setEqForm({...eqForm, unit: e.target.value})} /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Setor</label><input type="text" className="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium shadow-sm" placeholder="Ex: Comercial" value={eqForm.department} onChange={(e) => setEqForm({...eqForm, department: e.target.value})} /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Operador</label><input type="text" className="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium shadow-sm" placeholder="Ex: João Silva" value={eqForm.operator} onChange={(e) => setEqForm({...eqForm, operator: e.target.value})} /></div><div className="flex gap-4"><div className="flex-1 space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Endereço IP</label><input type="text" className="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium shadow-sm font-mono" placeholder="Ex: 192.168.1.100" value={eqForm.ipAddress} onChange={(e) => setEqForm({...eqForm, ipAddress: e.target.value})} /></div><div className="flex-1 space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">AnyDesk ID</label><input type="text" className="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:text-white font-medium shadow-sm font-mono" placeholder="Ex: 123456789" value={eqForm.anydeskId} onChange={(e) => setEqForm({...eqForm, anydeskId: e.target.value.replace(/\D/g, '')})} /></div></div></div>{eqForm.status !== 'baixado' && (<div className="space-y-2 mt-2"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Status Atual</label><div className="flex gap-2"><button type="button" onClick={() => setEqForm({...eqForm, status: 'guardado'})} className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${eqForm.status === 'guardado' ? 'bg-slate-800 text-white shadow-sm' : 'bg-white dark:bg-slate-800 text-slate-500 hover:bg-slate-100'}`}>Guardado / Estoque</button><button type="button" onClick={() => setEqForm({...eqForm, status: 'em_uso'})} className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${eqForm.status === 'em_uso' ? 'bg-blue-600 text-white shadow-sm' : 'bg-white dark:bg-slate-800 text-slate-500 hover:bg-slate-100'}`}>Em Uso</button></div></div>)}</div><div className="pt-4 flex justify-end gap-3"><button type="button" onClick={() => setIsEqModalOpen(false)} className="px-6 py-3 rounded-xl font-bold text-sm text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">Cancelar</button><button type="submit" disabled={isSubmitting} className="px-6 py-3 rounded-xl font-bold text-sm bg-blue-600 text-white shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all disabled:opacity-50">{editingEqId ? 'Salvar Alterações' : 'Salvar Equipamento'}</button></div></form></div></div></div>)}
            ...
            <header className="bg-white border-b sticky top-0 z-30 shadow-sm dark:bg-slate-900 dark:border-slate-800">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {isAdmin && (<button onClick={() => setIsAnnouncementModalOpen(true)} className="p-2 rounded-xl transition-all text-white bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/20 active:scale-95" title="Avisos Globais"><Megaphone size={20} /></button>)}
            ...
            {announcements.length > 0 && (
                <div className="w-full flex flex-col z-20 sticky top-16 shadow-md border-b border-transparent dark:border-slate-800">
                    {announcements.map(aviso => (
                        <div key={aviso.id} className={`w-full px-4 py-2.5 text-center animate-in fade-in slide-in-from-top-2 ${aviso.type === 'critical' ? 'bg-red-600 text-white' : aviso.type === 'warning' ? 'bg-orange-500 text-white' : 'bg-blue-600 text-white'}`}>
                            <div className="inline-flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-sm">
                                <span className="inline-flex items-center gap-1.5 align-middle">
                                    {aviso.type === 'critical' ? <AlertOctagon size={16} className="animate-pulse shrink-0" /> : aviso.type === 'warning' ? <AlertTriangle size={16} className="shrink-0" /> : <Info size={16} className="shrink-0" />}
                                    <span className="font-black uppercase tracking-widest text-[9px] bg-black/20 px-1.5 py-0.5 rounded leading-none shrink-0">{aviso.type === 'critical' ? 'Crítico' : aviso.type === 'warning' ? 'Atenção' : 'Info'}</span>
                                    <span className="font-bold ml-0.5">{aviso.title}:</span>
                                </span>
                                <span className="font-medium opacity-95 leading-snug">{aviso.message}</span>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <main className="flex-1 max-w-6xl mx-auto px-4 py-8 w-full">
                    ...
                    <div className="h-[calc(100vh-140px)] flex flex-col animate-in fade-in duration-500">
                        ...
                        {kanbanTab === 'kb' && (
                            <div className="flex-1 overflow-y-auto animate-in slide-in-from-right-4">
                                <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-1">
                                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest hidden md:block">Base de Conhecimento</p>
                                <div className="flex w-full md:w-auto items-center gap-3">
                                    <div className="relative flex-1 md:w-64">
                                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={14} />
                                        <input type="text" placeholder="Pesquisar informações..." className="w-full pl-9 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-medium outline-none focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:text-white transition-all shadow-sm" value={kbSearchTerm} onChange={(e) => setKbSearchTerm(e.target.value)} />
                                    </div>
                                    <button onClick={() => { setSelectedKbCard({ title: '', description: '', attachments: [] }); setIsKbCardModalOpen(true); }} className="flex-shrink-0 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 px-4 py-2.5 rounded-xl shadow-md shadow-blue-500/20 transition-all flex items-center gap-1.5"><Plus size={14}/> Nova Info</button>
                                </div>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                {kbCards.filter(card => card.title.toLowerCase().includes(kbSearchTerm.toLowerCase()) || card.description.toLowerCase().includes(kbSearchTerm.toLowerCase())).length === 0 ? (
                                    <div className="col-span-full py-12 text-center border-2 border-dashed rounded-3xl text-slate-400 font-bold border-slate-200 dark:border-slate-800">
                                        {kbCards.length === 0 ? 'Nenhuma informação registrada.' : 'Nenhuma informação encontrada para esta pesquisa.'}
                                    </div>
                                ) : (
                                    kbCards.filter(card => card.title.toLowerCase().includes(kbSearchTerm.toLowerCase()) || card.description.toLowerCase().includes(kbSearchTerm.toLowerCase())).map(card => (
                                        <div key={card.id} onClick={() => { setSelectedKbCard(card); setIsKbCardModalOpen(true); }} className="bg-white dark:bg-slate-900 p-5 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 cursor-pointer hover:shadow-md hover:-translate-y-1 transition-all group flex flex-col h-48">
                                            <div className="flex items-center gap-3 mb-3"><div className="p-2.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl"><BookOpen size={20}/></div><h4 className="text-sm font-bold text-slate-800 dark:text-white leading-snug flex-1 line-clamp-2">{card.title}</h4></div>
                ...
                <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
                    ...
                    <div className="space-y-4 animate-in slide-in-from-left-4">
                        ...
                        <button onClick={() => { setEditingEqId(null); setEqForm({ type: 'Notebook', model: '', specs: '', acquisitionDate: '', warrantyDate: '', price: '', deviceId: '', serialNumber: '', ipAddress: '', status: 'guardado', unit: '', department: '', operator: '', anydeskId: '' }); setIsEqModalOpen(true); }} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg shadow-blue-500/20 whitespace-nowrap ml-4"><Plus size={16} /> Novo Equipamento</button>
                        </div>
                        ...
                        {filteredEquipments.length === 0 ? (<div className="py-24 text-center border-2 border-dashed rounded-3xl text-slate-400 font-bold border-slate-200 dark:border-slate-800">Nenhum equipamento ativo encontrado.</div>) : (
                            ...
                            <div key={eq.id} className="bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-3xl p-5 shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="text-lg font-black dark:text-white leading-tight">{eq.model}</h4>
                                        <div className="flex items-center flex-wrap gap-2 mt-1">
                                            <span className="text-[10px] font-bold text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md uppercase tracking-wider">{eq.type}</span>
                                            {eq.deviceId && <span className="text-[10px] font-mono font-bold text-slate-400 px-2 py-0.5 border dark:border-slate-700 rounded-md"><Hash size={10} className="inline mr-1 -mt-0.5"/>{eq.deviceId}</span>}
                                            {eq.serialNumber && <span className="text-[10px] font-mono font-bold text-slate-400 px-2 py-0.5 border dark:border-slate-700 rounded-md">S/N: {eq.serialNumber}</span>}
                                        </div>
                                    </div>
                                </div>
                                <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-4 px-0 lg:px-6">
                                    ...
                                    <div><p className="text-[9px] font-black uppercase text-slate-400 mb-0.5">Nº de Série</p><p className="text-xs font-bold dark:text-slate-300 font-mono truncate" title={eq.serialNumber}>{eq.serialNumber || 'N/A'}</p></div>
                                    ...
                                    <div><p className="text-[9px] font-black uppercase text-slate-400 mb-0.5">Garantia</p><p className="text-xs font-bold dark:text-slate-300">{eq.warrantyDate ? formatDateBr(eq.warrantyDate) : 'N/A'}</p></div>
                            ...
                            <div key={eq.id} className="bg-red-50/50 dark:bg-slate-900 border border-red-200 dark:border-red-900/50 rounded-3xl p-5 shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <div className="flex items-center gap-4 opacity-80">
                                    ...
                                    <div>
                                        <h4 className="text-lg font-black dark:text-white leading-tight">{eq.model}</h4>
                                        <div className="flex items-center flex-wrap gap-2 mt-1">
                                            <span className="text-[10px] font-bold text-slate-500 bg-white dark:bg-slate-800 px-2 py-0.5 rounded-md uppercase tracking-wider">{eq.type}</span>
                                            {eq.deviceId && <span className="text-[10px] font-mono font-bold text-slate-400 px-2 py-0.5 border border-red-100 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-md"><Hash size={10} className="inline mr-1 -mt-0.5"/>{eq.deviceId}</span>}
                                            {eq.serialNumber && <span className="text-[10px] font-mono font-bold text-slate-400 px-2 py-0.5 border border-red-100 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-md">S/N: {eq.serialNumber}</span>}
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="space-y-4 animate-in slide-in-from-right-4">
                        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-slate-900 dark:bg-slate-800 p-4 rounded-2xl shadow-sm gap-4">
                        <div className="flex gap-4 text-white min-w-max">
                            <div className="text-center px-4">
                                <p className="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">Itens Encontrados</p>
                                <p className="text-2xl font-black">{filteredAccessories.length}</p>
                            </div>
                        </div>
                        <div className="flex flex-1 w-full lg:w-auto justify-center">
                            <div className="flex bg-slate-800 dark:bg-slate-900 p-1 rounded-xl w-full sm:w-auto">
                                <button onClick={() => setAccViewMode('stock')} className={`flex-1 px-6 py-2 rounded-lg text-xs font-bold transition-all ${accViewMode === 'stock' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}>Estoque Atual</button>
                                <button onClick={() => setAccViewMode('movements')} className={`flex-1 px-6 py-2 rounded-lg text-xs font-bold transition-all ${accViewMode === 'movements' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}>Relatório de Movimentações</button>
                            </div>
                        </div>
                        <button onClick={() => setIsAccModalOpen(true)} className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg shadow-blue-500/20 w-full lg:w-auto shrink-0"><Plus size={16} /> Novo Acessório</button>
                        </div>
                        ...
                        {accViewMode === 'stock' && (
                            ...
                            {filteredAccessories.length === 0 ? (<div className="col-span-full py-24 text-center border-2 border-dashed rounded-3xl text-slate-400 font-bold border-slate-200 dark:border-slate-800">Nenhum acessório encontrado.</div>) : (
                                ...
                                {expandedLogId === acc.id && (
                                    <div className="mt-4 pt-4 border-t dark:border-slate-800 animate-in zoom-in-95">
                                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">Últimas 5 Movs.</p>
                                        <div className="space-y-2">
                                            {[...(acc.history || [])].reverse().slice(0,5).map((log, i) => {
                                                const actionLower = (log.action || '').toLowerCase();
                                                const isEntrada = actionLower.includes('entrada') || actionLower.includes('cadastrado') || actionLower.includes('inicial');
                                                const isSaida = actionLower.includes('saída') || actionLower.includes('saida');
                                                return (
                                                    <div key={i} className="text-[10px]">
                                                        <p className={`font-bold ${isEntrada ? 'text-green-700 dark:text-green-400' : isSaida ? 'text-orange-700 dark:text-orange-400' : 'text-slate-700 dark:text-slate-300'} leading-tight`}>{log.action}</p>
                                                        <p className="text-slate-400 mt-0.5">{formatTimestamp(log.date)} • {log.user}</p>
                                                    </div>
                                                );
                                            })}
                                        </div>
                        ...
                        {accViewMode === 'movements' && (
                            <div className="space-y-4 animate-in fade-in zoom-in-95">
                                <div className="flex flex-wrap items-center gap-4 bg-white dark:bg-slate-900 border dark:border-slate-800 p-4 rounded-2xl shadow-sm">
                                    <p className="text-xs font-bold uppercase tracking-widest text-slate-400 flex items-center gap-2"><History size={16}/> Filtros:</p>
                                    <div className="relative" ref={accMoveTypeMenuRef}>
                                        <button onClick={() => setIsAccMoveTypeMenuOpen(!isAccMoveTypeMenuOpen)} className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-xs font-bold flex items-center justify-between gap-2 outline-none focus:ring-2 focus:ring-blue-500 transition-all dark:text-white min-w-[140px]"><Filter size={14} className="text-blue-500 shrink-0"/><span className="truncate">{accMoveTypeFilter === 'Todos' ? 'Todos os Tipos' : accMoveTypeFilter}</span><ChevronDown size={14} className={`transition-transform flex-shrink-0 ${isAccMoveTypeMenuOpen ? 'rotate-180' : ''}`} /></button>
                                        {isAccMoveTypeMenuOpen && (<div className="absolute z-50 mt-2 w-full bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden py-1 animate-in fade-in zoom-in-95 duration-200 custom-scrollbar max-h-60 overflow-y-auto"><button onClick={() => { setAccMoveTypeFilter('Todos'); setIsAccMoveTypeMenuOpen(false); }} className="w-full px-4 py-2 text-left text-xs font-bold hover:bg-blue-50 dark:hover:bg-blue-900/20 dark:text-white flex items-center justify-between">Todos os Tipos {accMoveTypeFilter === 'Todos' && <Check size={12} className="text-blue-600" />}</button><div className="h-px bg-slate-100 dark:bg-slate-800 mx-2 my-1" />{ACCESSORY_TYPES.map(t => (<button key={t} onClick={() => { setAccMoveTypeFilter(t); setIsAccMoveTypeMenuOpen(false); }} className="w-full px-4 py-2 text-left text-xs font-bold hover:bg-blue-50 dark:hover:bg-blue-900/20 dark:text-white flex items-center justify-between">{t} {accMoveTypeFilter === t && <Check size={12} className="text-blue-600" />}</button>))}</div>)}
                                    </div>
                                    <div className="relative" ref={accMoveMonthMenuRef}>
                                        <button onClick={() => setIsAccMoveMonthMenuOpen(!isAccMoveMonthMenuOpen)} className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-xs font-bold flex items-center justify-between gap-2 outline-none focus:ring-2 focus:ring-blue-500 transition-all dark:text-white min-w-[140px]"><Calendar size={14} className="text-blue-500 shrink-0"/><span className="truncate">{accMoveMonth === 'all' ? 'Todos os Meses' : MONTHS[accMoveMonth]}</span><ChevronDown size={14} className={`transition-transform flex-shrink-0 ${isAccMoveMonthMenuOpen ? 'rotate-180' : ''}`} /></button>
                                        {isAccMoveMonthMenuOpen && (<div className="absolute z-50 mt-2 w-full bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden py-1 animate-in fade-in zoom-in-95 duration-200 custom-scrollbar max-h-60 overflow-y-auto"><button onClick={() => { setAccMoveMonth('all'); setIsAccMoveMonthMenuOpen(false); }} className="w-full px-4 py-2 text-left text-xs font-bold hover:bg-blue-50 dark:hover:bg-blue-900/20 dark:text-white flex items-center justify-between">Todos os Meses {accMoveMonth === 'all' && <Check size={12} className="text-blue-600" />}</button><div className="h-px bg-slate-100 dark:bg-slate-800 mx-2 my-1" />{MONTHS.map((m, i) => (<button key={i} onClick={() => { setAccMoveMonth(i); setIsAccMoveMonthMenuOpen(false); }} className="w-full px-4 py-2 text-left text-xs font-bold hover:bg-blue-50 dark:hover:bg-blue-900/20 dark:text-white flex items-center justify-between">{m} {accMoveMonth === i && <Check size={12} className="text-blue-600" />}</button>))}</div>)}
                                    </div>
                                    <div className="relative" ref={accMoveYearMenuRef}>
                                        <button onClick={() => setIsAccMoveYearMenuOpen(!isAccMoveYearMenuOpen)} className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-xs font-bold flex items-center justify-between gap-2 outline-none focus:ring-2 focus:ring-blue-500 transition-all dark:text-white min-w-[100px]"><span className="truncate">{accMoveYear}</span><ChevronDown size={14} className={`transition-transform flex-shrink-0 ${isAccMoveYearMenuOpen ? 'rotate-180' : ''}`} /></button>
                                        {isAccMoveYearMenuOpen && (<div className="absolute z-50 mt-2 w-full bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden py-1 animate-in fade-in zoom-in-95 duration-200 custom-scrollbar max-h-60 overflow-y-auto">{availableYears.map(y => (<button key={y} onClick={() => { setAccMoveYear(y); setIsAccMoveYearMenuOpen(false); }} className="w-full px-4 py-2 text-left text-xs font-bold hover:bg-blue-50 dark:hover:bg-blue-900/20 dark:text-white flex items-center justify-between">{y} {accMoveYear === y && <Check size={12} className="text-blue-600" />}</button>))}</div>)}
                                    </div>
                                </div>
                                ...
                                <div className="bg-white dark:bg-slate-900 rounded-3xl border dark:border-slate-800 overflow-hidden shadow-sm">
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left text-sm">
                                            <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 uppercase text-[10px] tracking-widest font-black">
                                                <tr>
                                                    <th className="px-6 py-4">Data e Hora</th>
                                                    <th className="px-6 py-4">Ação / Movimentação</th>
                                                    <th className="px-6 py-4">Técnico</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y dark:divide-slate-800">
                                                {groupedMovements.length === 0 ? (
                                                    <tr><td colSpan="3" className="px-6 py-12 text-center text-slate-400 font-bold border-2 border-dashed border-transparent">Nenhuma movimentação registrada no período.</td></tr>
                                                ) : (
                                                    groupedMovements.map(group => (
                                                        <React.Fragment key={group.accId}>
                                                            <tr className="bg-slate-100/50 dark:bg-slate-800/50 border-y dark:border-slate-700/50 cursor-pointer hover:bg-slate-200/50 dark:hover:bg-slate-800/80 transition-colors" onClick={() => setExpandedAccGroups(prev => ({...prev, [group.accId]: !prev[group.accId]}))}>
                                                                <td colSpan="3" className="px-6 py-3">
                                                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                                                        <div className="flex items-center gap-3">
                                                                            <Layers size={16} className="text-blue-500 shrink-0"/>
                                                                            <span className="font-bold text-slate-800 dark:text-white text-sm">{group.accName}</span>
                                                                            <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 bg-white dark:bg-slate-900 border dark:border-slate-700 px-2 py-0.5 rounded-md shrink-0">{group.accType}</span>
                                                                        </div>
                                                                        <div className="flex flex-wrap items-center gap-4">
                                                                            <div className="flex flex-wrap items-center gap-2">
                                                                                {group.entradas > 0 && <span className="text-[10px] font-bold text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/30 px-2 py-1 rounded-md flex items-center gap-1"><Plus size={12}/>{group.entradas} Entradas</span>}
                                                                                {group.saidas > 0 && <span className="text-[10px] font-bold text-orange-700 dark:text-orange-400 bg-orange-100 dark:bg-orange-900/30 px-2 py-1 rounded-md flex items-center gap-1"><Minus size={12}/>{group.saidas} Saídas</span>}
                                                                            </div>
                                                                            <button type="button" className="text-[10px] font-bold text-blue-600 flex items-center gap-1 bg-white dark:bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">
                                                                                {expandedAccGroups[group.accId] ? 'Ocultar Detalhes' : 'Ver Detalhes'}
                                                                                <ChevronDown size={14} className={`transition-transform duration-300 ${expandedAccGroups[group.accId] ? 'rotate-180' : ''}`} />
                                                                            </button>
                                                                        </div>
                                                                    </div>
                                                                </td>
                                                            </tr>
                                                            {expandedAccGroups[group.accId] && group.logs.map((log, i) => {
                                                                const actionLower = (log.action || '').toLowerCase();
                                                                const isEntrada = actionLower.includes('entrada') || actionLower.includes('cadastrado') || actionLower.includes('inicial');
                                                                const isSaida = actionLower.includes('saída') || actionLower.includes('saida');
                                                                return (
                                                                    <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors animate-in fade-in slide-in-from-top-2">
                                                                        <td className="px-6 py-3 whitespace-nowrap text-xs font-medium text-slate-500 dark:text-slate-400 lg:pl-12">{formatTimestamp(log.date)}</td>
                                                                        <td className="px-6 py-3 min-w-[300px]">
                                                                            <div className="flex items-start gap-2">
                                                                                <div className={`w-2 h-2 mt-1 rounded-full shrink-0 ${isEntrada ? 'bg-green-500' : isSaida ? 'bg-orange-500' : 'bg-slate-400'}`}></div>
                                                                                <p className={`text-xs font-bold leading-relaxed ${isEntrada ? 'text-green-700 dark:text-green-400' : isSaida ? 'text-orange-700 dark:text-orange-400' : 'text-slate-700 dark:text-slate-300'}`}>{log.action}</p>
                                                                            </div>
                                                                        </td>
                                                                        <td className="px-6 py-3 whitespace-nowrap text-xs font-bold dark:text-slate-300"><User size={12} className="inline mr-1 opacity-50"/>{log.user}</td>
                                                                    </tr>
                                                                );
                                                            })}
                                                        </React.Fragment>
